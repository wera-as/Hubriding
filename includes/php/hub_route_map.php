		...
		while (have_rows('rute_rutekart')) {
			...
			if (isset($unique_hotels[$map_hotel->post_name])) {
				if ($map_type == "ridewithgps") {
					...
					$content .= "<iframe src='" . $map_gps_id_prepend . $map_gps_id . $map_gps_id_append . "' class='hub-map-container hub-route-display $map_hotel->post_name' style='width: 1px; min-width: 100%; height: " . $iframe_heigth . "px; border: none;' scrolling='no'></iframe>";
				} elseif ($map_type == "googlemaps") {
					$map_google_id_prepend = 'https://www.google.com/maps/embed?';
					$content .= "<iframe src='" . $map_google_id_prepend . $map_google_id . "' class='hub-map-container hub-route-display $map_hotel->post_name' width='600' height='$iframe_heigth' frameborder='0' style='border:0;' allowfullscreen='' aria-hidden='false' tabindex='0'></iframe>";
				} elseif ($map_type == "bilde") {
